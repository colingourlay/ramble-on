import { autoLinkEntities, extractEntitiesWithIndices, getTweetLength, htmlEscape } from 'twitter-text';
...
function transform(fullText, config) {
  ...
  const sections = fullText
    .split(/[\r\n]+[\r\n]+/g)
    .map(text => {
      return { text: text.trim(), isPar: true };
    })
...
export { COUNTERS, DECORATORS, example };
