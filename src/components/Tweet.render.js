...
export default function render(tweet, index, tweets) {
  ...
  const countStyleName = tweets.length > 99 ? 'countTriple' : tweets.length > 9 ? 'countDouble' : 'count';
  const remaining = 140 - tweet.length;
  const remainingStyleName = remaining < 10 ? 'remainingSingle' : remaining < 100 ? 'remainingDouble' : 'remaining';
