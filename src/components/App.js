...
function App(initialState) {
    ...
    initialState.counterName =
        initialState.counterName || Object.keys(transform.COUNTERS)[0];
    initialState.decoratorName =
        initialState.decoratorName || Object.keys(transform.DECORATORS)[0];
...
function post(model) {
    ...
    function next(response) {
        ...
        if (response && model.numTweetsPosted() === 1) {
            initialTweetUrl = `http://twitter.com/${response.user
                .screen_name}/status/${response.id_str}`;
        }
        ...
        config.data.message = encodeURIComponent(
            tweets[model.numTweetsPosted()].text
        );
