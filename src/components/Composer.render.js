...
export default function render(state) {
  return h('div.root', [
    partial(renderText, state.isPosting, state.text, state.channels.setText),
    h('label.counterLabel', 'Counter'),
    partial(renderCounter, state.isPosting, state.counterName, state.channels.setCounterName),
    h('label.decoratorLabel', 'Style'),
    partial(renderDecorator, state.isPosting, state.decoratorName, state.counterName, state.channels.setDecoratorName),
    partial(renderPost, state.isPosting, state.text, state.tweets, state.numTweetsPosted, state.channels.post),
    partial(renderTiming, state.isPosting, state.tweets, state.timing, state.channels.setTiming)
  ]);
}
...
function renderTiming(isPosting, tweets, timing, channel) {
  return tweets.length > 1
    ? h('div.timingContainer', [
        h('label.timingLabel', `Wait ${humanize(timing, { round: true })} between posts`),
        h('input.timing', {
          name: 'timing',
          type: 'range',
          min: 0,
...
function renderPost(isPosting, text, tweets, numTweetsPosted, channel) {
  ...
  const reaction =
    tweets.length < 10
      ...
      : tweets.length < 15
        ? 'seriously?'
        : tweets.length < 20 ? 'whoa there!' : tweets.length < 25 ? 'stop. just, stop.' : 'OH, COME ON!';
  ...
  return text.length
    ? h(
        ...
        isPosting
          ? `Posting ${numTweetsPosted + 1}${tweets.length > 1 ? ' of ' + tweets.length : ''} ${pTweets}...`
